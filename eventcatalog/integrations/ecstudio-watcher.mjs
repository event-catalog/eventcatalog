import { logger } from '../src/utils/cli-logger';
        ...
        server.watcher
          .on('add', async (path) => {
            if (isEcstudioFile(path)) {
              logger.info(`New .ecstudio file detected: ${path}`);
              ...
              if (refreshContent) {
                try {
                  ...
                  logger.success('Content refreshed after adding new .ecstudio file');
                } catch (error) {
          ...
          .on('unlink', async (path) => {
            if (isEcstudioFile(path)) {
              logger.info(`Removed .ecstudio file: ${path}`);

              if (refreshContent) {
                try {
                  await refreshContent();
                  logger.success('Content refreshed after removing .ecstudio file');
                } catch (error) {
                  logger.error(`Error refreshing content: ${error}`);
                }
              }
          ...
          .on('change', async (path) => {
            if (isEcstudioFile(path)) {
              logger.info(`Changed .ecstudio file: ${path}`);
              await refreshContent();
            }
          });
