...
import { createEdge, generatedIdForEdge, generateIdForNode, getEdgeLabelForMessageAsSource, getEdgeLabelForServiceAsTarget, getNodesAndEdgesFromDagre } from '@utils/node-graphs/utils/utils';
...
export const useEventCatalogVisualiser = ({ nodes, edges, setNodes, setEdges }: EventCatalogVisualizerProps) => {
    ...
    const updatedEdges = useMemo(() => {
        return edges.reduce<Edge[]>((acc, edge) => {
            ...
            if (sourceIsChannel) {
                const edgeLabel = data.target.collection === 'services' ? getEdgeLabelForMessageAsSource(data.source) : getEdgeLabelForServiceAsTarget(data.target);
