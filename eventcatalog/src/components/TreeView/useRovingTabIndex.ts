...
import { FocusKeys, useFocusZone } from './useFocusZone';
...
export function useRovingTabIndex({
  ...
  useFocusZone({
    containerRef,
    bindKeys:
      FocusKeys.ArrowVertical | FocusKeys.ArrowHorizontal | FocusKeys.HomeAndEnd | FocusKeys.Backspace | FocusKeys.PageUpDown,
    preventScroll: true,
...
export function getLastElement(element: HTMLElement): HTMLElement | undefined {
  ...
  while (index > 0 && last instanceof HTMLElement && last.parentElement?.closest('[role=treeitem][aria-expanded=false]')) {
    index -= 1;
    last = items[index];
  }
