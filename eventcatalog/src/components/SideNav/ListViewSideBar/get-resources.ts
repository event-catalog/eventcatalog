...
export async function getCatalogResources({ currentPath }: { currentPath: string }) {
  ...
  const messagesNotInService = messages.filter((message) => !services.some((service) => service.data?.sends?.some((send: any) => send.data.id === message.data.id) || service.data?.receives?.some((receive: any) => receive.data.id === message.data.id)));
  ...
  console.log('messagesNotInServiceAsSideNav', messagesNotInServiceAsSideNav)
