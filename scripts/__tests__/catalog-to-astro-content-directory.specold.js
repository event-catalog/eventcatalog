...
describe('catalog-to-astro-content-directory', () => {
  beforeAll(() => {
    ...
    fs.ensureDirSync(ASTRO_OUTPUT);
    fs.writeFileSync(path.join(ASTRO_OUTPUT, 'config.ts'), 'export const config = {};');
  });
  ...
  describe('events', () => {
    describe('/events directory', () => {
      it('takes an event from the users catalog and puts it into the expected directory structure', async () => {
        const source = path.join(__dirname, 'catalog-example');

        await catalogToAstro(source, ASTRO_OUTPUT, OUTPUT_EXTERNAL_FILES);

        expect(fs.existsSync(path.join(ASTRO_OUTPUT, 'events', 'PaymentProcessed', 'index.mdx'))).toBe(true);
      });
