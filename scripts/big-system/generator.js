...
const catalogBasePath = join(rootDir, '/scripts/big-system/');
...
const createGenerators = (specpath, generator) => {
  ...
  const schemas = globSync(specpath)?.sort((a, b) => b.localeCompare(a) ).reverse();
  if (!schemas) return [];
...
for(let i = 0 ; i < 500; i++) {
  ...
  const copyFolderRecursiveSync = (target) => {
    ...
    fs.writeFileSync(
      `${destinationFolder}/APP.yaml`, 
      sourceContent.replace(/<APP>/g, i.toString()),
      'utf8');
  };
...
fs.writeFileSync(join(catalogPath, 'eventcatalog.config.js'), 
  `/** @type {import('@eventcatalog/core/bin/eventcatalog.config').Config} */
export default ${JSON.stringify({ ...baseConfig, generators }, null, 2)}`, 'utf8');
