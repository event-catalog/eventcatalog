...
<Tiles>
  <Tile icon="BoltIcon" href={`/visualiser/services/${frontmatter.id}/${frontmatter.version}`} title={`Receives ${frontmatter.receives.length} message`} description="The checkout command from Shopping" />
  <Tile icon="RectangleGroupIcon" href="/docs/services/checkout-orchestrator/1.0.0" title="Delegates to the Orchestrator" description="The saga that runs checkout" />
</Tiles>
