...
- 2c2cea7: Read catalog content and customizations directly from the project without creating a `.eventcatalog-core` directory. After stopping older EventCatalog processes, the unused `.eventcatalog-core/` directory is safe to delete.
...
- 5269292: Fix environment switcher to support catalogs hosted under different base paths. The environment dropdown now matches the current environment by origin and base path, and preserves the in-catalog path, search params, and hash when switching environments.
...
- f581840: Expand sidebar and search navigation: link messages and services to the flows they appear in, surface flow steps (messages, services, subflows) on flow pages, add entities to the global sidebar, and include data products, flows, and entities in the search modal with correct icons, filters, and URLs.

## 3.34.0

### Minor Changes

- f2f4a48: Upgrade Astro and related integrations to the latest 6.3.x line (astro 6.3.1, @astrojs/check 0.9.9, @astrojs/markdown-remark 7.1.1, @astrojs/mdx 5.0.4, @astrojs/node 10.1.0, @astrojs/react 5.0.4). Align `@types/react` / `@types/react-dom` on v19 so type-checking works across the workspace. Replace `getEntry('customPages', …)` with a cached `getCollection` lookup in the custom-documentation nav builder to avoid Astro's noisy `slug` deprecation accessor firing thousands of times per build.
...
- 31ffbd9: fix: render JSON Schema array-valued `type` correctly in `SchemaViewer`. Types declared as arrays (e.g. `["object", "null"]`) now display as `object | null` instead of `objectnull`, and nested properties/array items are still traversable when the type is a union.
...
- b8734d4: feat(llm): include resource docs in llms.txt and expose raw markdown routes
...
- 3ea125d: allow scalar configuration to be passed through to the OpenAPI spec component via `scalarConfiguration` config option
