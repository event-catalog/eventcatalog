import { defineConfig } from '@eventcatalog/auth-astro';
...
const getAuthProviders = async () => {
  try {
    ...
    if (authConfig.providers?.github) {
      ...
      // validation works even if a catalog still resolves an older @auth/core than the
      // @eventcatalog/auth-astro peer (>=0.41.3) that EventCatalog ships.
