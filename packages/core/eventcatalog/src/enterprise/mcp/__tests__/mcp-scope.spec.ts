...
describe('resolveMcpScope', () => {
  it('resolves the latest domain and recursively includes its contained resource graph', async () => {
    const scope = await resolveMcpScope({ kind: 'domain', id: 'payments' }, loadCollection);
  ...
  it('supports a system as the root scope', async () => {
    const scope = await resolveMcpScope({ kind: 'system', id: 'payment-system', version: '1.0.0' }, loadCollection);
  ...
  it('fails cleanly when the requested scope does not exist', async () => {
    await expect(resolveMcpScope({ kind: 'domain', id: 'missing' }, loadCollection)).rejects.toBeInstanceOf(
