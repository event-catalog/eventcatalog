...
const processAutoGeneratedDirectory = async (
  ...
  // @ts-ignore
  const items = fs.readdirSync(path.join(getContentDir(), DOCS_DIR, directory));
  ...
  if (indexFile) {
    const indexPath = path.join(getContentDir(), DOCS_DIR, directory, indexFile);
  ...
  for (const item of items) {
    ...
    const fullPath = path.join(getContentDir(), DOCS_DIR, directory, item);
