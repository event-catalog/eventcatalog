...
When you define a message (command, query or event) in EventCatalog, you can also specify how this message is transported (channel).

You do this using the `to` and `from` fields in your producer/consumer frontmatter.

### Publishing messages to channels

To publish a message through a channel you need to specify the channel in the `to` field in your producers frontmatter.

In this example the `OrderService` (service) sends an `OrderPlaced` message to the `orders.events` channel.
...
To consume a message from a channel you need to specify the channel in the `from` field in your consumer frontmatter.

In this example the `PaymentService` (service) consumes the `OrderPlaced` message from the `orders.events` channel.
