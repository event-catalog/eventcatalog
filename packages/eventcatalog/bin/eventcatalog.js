...
cli
  ...
  .action((siteDir, { port, host, locale, config, hotOnly, open, poll }) => {
    ...
    // copy the build back...
    fs.copySync(projectBuildDir, path.join(__dirname, '../.next'), {overwrite: true}, function (err) {
      if (err) {                 
        console.error(err);   
      } else {
        console.log("success!");
...
cli
  ...
  .action((siteDir, { port, host, locale, config, hotOnly, open, poll }) => {
    ...
    fs.copySync(path.join(__dirname, '../.next'), projectBuildDir, {overwrite: true}, function (err) {
      if (err) {                 
        console.error(err);   
      } else {
        console.log("success!");
