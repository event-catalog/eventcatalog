...
import { ChevronDownIcon, FilterIcon, MinusSmIcon, PlusSmIcon, ViewGridIcon, ThumbUpIcon, EyeIcon, ChatAltIcon, ShareIcon } from '@heroicons/react/solid';
  ...
  const filters = [
    {
      id: 'domains',
      name: 'Domains',
      options: domains.map((domain) => ({ value: domain, label: domain, checked: false })),
    },
    {
      id: 'services',
      name: 'Services',
      options: services.map((service) => ({ value: service, label: service, checked: false })),
    },
  ];
          ...
          <section aria-labelledby="products-heading" className="pt-6 pb-24">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-x-8 gap-y-10">
              ...
              <div className="lg:col-span-3">
                ...
                <div>
                  <h2 className="text-gray-500 text-xs font-medium uppercase tracking-wide">Events / Messages</h2>
                      ...
                      <li key={event.name}>
                        <Link href={`/events/${event.name}`}>
                          <a className="col-span-1 flex shadow-sm rounded-md">
                            <div style={{background: getBackgroundColor(event.domains[0])}} className={classNames('bg-red-500', 'flex-shrink-0 flex items-center justify-center w-4 text-white text-sm font-medium rounded-l-md')}>
                            ...
                            <div className="w-full items-center justify-between border-t border-r border-b border-gray-200 bg-white rounded-r-md relative">
                              <div className="px-4 py-2 text-sm space-y-2">
                                <a href={event.href} className="text-gray-900 font-bold hover:text-gray-600">
                                  {event.name}
                                  <span className="ml-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">v{event.version}</span>
                                </a>
                                <div className="text-gray-500 text-xs block">Event is created when the user logs into the account.</div>
...
export const getServerSideProps = () => {
  ...
  const files = folders.map((folder) => fs.readFileSync(path.join(projectDir, 'events', folder, 'index.md'), { encoding: 'utf-8' }));
