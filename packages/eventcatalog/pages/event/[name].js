...
async function getEventById(eventName) {
  ...
  return { event: eventName, schema, ...data, content, source: mdxSource, lastModifiedDate: `${lastModifiedDate.getFullYear()}/${lastModifiedDate.getMonth()+1}/${lastModifiedDate.getDate()}` };
}
...
export async function getStaticPaths() {
  ...
  const files = folders.map((folder) => readFileSync(join(projectDir, 'events', folder, 'index.md'), { encoding: 'utf-8' }));
