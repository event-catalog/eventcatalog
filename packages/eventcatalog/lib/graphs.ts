...
export const buildMermaidFlowChartForEvent = (
  ...
  producerNames
    .forEach((producer) => {
      const eventNode = node(producer, generateLink(producer, 'services', producers.find((item) => item.name === producer)?.domain))
      builder.addProducerFlow(eventNode, centerNode)
    })
  consumerNames
    .forEach((consumer) => {
      const eventNode = node(consumer, generateLink(consumer, 'services', consumers.find((item) => item.name === consumer)?.domain))
