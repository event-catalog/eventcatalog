...
export const StatusBar = memo(function StatusBar({
  ...
  const counts = useMemo(() => {
    ...
    return DISPLAY_ORDER.filter((type) => map[type]).map((type) => {
      const n = map[type];
    ...
    <footer className="status-bar">
      <div className="status-bar-left">
        ...
        {counts.length > 0 ? (
          ...
          <span className="status-bar-counts status-bar-empty">
      ...
      <div className="status-bar-right">
        <a
          ...
          href="https://github.com/event-catalog/eventcatalog/issues/new?labels=compass&template=bug_report.md"
          target="_blank"
