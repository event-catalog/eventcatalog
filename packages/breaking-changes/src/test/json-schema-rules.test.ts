  ...
  describe('nested schemas', () => {
    it('detects breaking changes in nested fields', () => {
      ...
      expect(result).toContainEqual(
        expect.objectContaining({ type: 'TYPE_CHANGED', field: 'address.street', breaking: true })
      );
      expect(result).toContainEqual(
        expect.objectContaining({ type: 'FIELD_REMOVED_REQUIRED', field: 'address.city', breaking: true })
      );
    });
  });
});
